...
// ----------------------------------------------------
// DATA NARASI TERBARU (SINKRONISASI TOTAL)
// ----------------------------------------------------
String[] narrFile = {
  "Narr-1.1.mp3", "Narr-1.2.mp3",   // Part 1.1, 1.2 (Intro)
  "Narr-2.1.mp3", "Narr-2.2.mp3",   // Part 2.1, 2.2 (Kawung)
  "Narr-3.1.mp3", "Narr-3.2.mp3",   // Part 3.1, 3.2 (Megamendung)
  "Narr-4.1.mp3", "Narr-4.2.mp3",   // Part 4.1, 4.2 (Parang) - Dimajukan!
  "Narr-ED1.mp3", "Narr-ED2.mp3"    // Ending Part 1, 2 - Dimajukan!
};

// Jadwal Audio Baru (Maju mengikuti Scene 9 yang jadi 7 detik)
int[] narrStart = {
  0,      16000,   // Intro
  24000,  34000,   // Kawung
  43000,  61000,   // Megamendung
  82000,  91000,   // Parang (Mulai tepat di 82.000 ms)
  99000,  107000   // Ending 
};
...
SoundFile bgm;
float volumeBGM = 0.02; // 0.0 - 1.0

// Titik "skip" untuk mouse (Sudah di-update)
int[] waypoints = {
...
void draw() {
  ...
  // ====================================================
  // TIMELINE BARU (FIX BUG PARANG TRANSPARAN)
  // ====================================================
  ...
  else if (waktu < 75000) scene = 8;       // Scene 8: 61k - 75k (14 Detik)
  else if (waktu < 82000) scene = 9;       // Scene 9: 75k - 82k (CUMA 7 DETIK)
  else if (waktu < 87000) scene = 10;      // Scene 10 (Parang): 82k - 87k
  else if (waktu < 90000) scene = 11;      // Scene 5.1: 87k - 90k
  else if (waktu < 92000) scene = 12;      // Scene 5.2: 90k - 92k
  else if (waktu < 95500) scene = 13;      // Scene 5.3: 92k - 95.5k
  else if (waktu < 99000) scene = 14;      // Scene 5.4: 95.5k - 99k
  else if (waktu < 115000) scene = 15;     // Scene Penutup: 99k - 115k (16 Detik)
  else scene = 16;                         // Scene Kredit: 115k ke atas
  ...
  else if (scene == 16) drawCreditScene(waktu);
...
// ----------------------------------------------------
// SCENE 1 - 7 (Tetap Sama)
// ----------------------------------------------------
void drawScene1() {
  background(#4F7942); imageMode(CENTER);
  if (alpha1 < 255) alpha1 += 2; zoom1 += 0.001;
  pushMatrix(); translate(width/2, height/2); scale(zoom1); tint(255, alpha1); image(img1, 0, 0); popMatrix();
}

void drawScene2() {
  background(#240902); imageMode(CORNER);
  if (alpha2 < 255) alpha2 += 3; tint(255, alpha2);
...
void drawScene3(int waktu) {
  background(#0B132B); imageMode(CENTER);
  if (waktu < 32000) { if (alpha3 < 255) alpha3 += 3; } else { if (alpha3 > 0) alpha3 -= 8; }
  tint(255, alpha3); image(img3, width/2, height/2, img3.width*0.6, img3.height*0.6);
  float aK = map(alpha3, 0, 255, 0, 127); float aI = map(alpha3, 0, 255, 0, 255);
  for (KunangKunang k : kumpulanKunang) { k.update(); k.display(aK, aI); }
  kunangSpesial.update(); kunangSpesial.display(aK, aI);
}

void drawScene32(int waktu) {
  background(0); imageMode(CENTER);
  if (waktu < 37000) { if (alpha32 < 255) alpha32 += 4; } else { if (alpha32 > 0) alpha32 -= 8; }
  tint(255, alpha32);
...
void drawScene33(int waktu) {
  background(#0B132B); imageMode(CENTER);
  if (waktu < 42000) { if (alpha33 < 255) alpha33 += 3; } else { if (alpha33 > 0) alpha33 -= 8; }
  tint(255, alpha33); image(img33, width/2, height/2, img33.width*1.0, img33.height*1.0);
  for (KunangKunang k : kumpulanKunang) { k.update(); k.display(map(alpha33, 0, 255, 0, 127), map(alpha33, 0, 255, 0, 255)); }
}

void drawSceneMegamendung(int waktu) {
  background(#0F2027); imageMode(CENTER);
  if (waktu < 55000) { if (alphaMega < 255) alphaMega += 10; } else { if (alphaMega > 0) alphaMega -= 15; }
  tint(255, alphaMega);
  for (AwanMegamendung a : kumpulanAwan) { a.display(); }
}

void drawScene41(int waktu) {
  background(#AFEEEE); imageMode(CENTER);
  if (waktu < 60000) { if (alpha41 < 255) alpha41 += 4; } else { if (alpha41 > 0) alpha41 -= 12; }
  tint(255, alpha41);
  image(img41, width/2, height/2, img41.width, img41.height);
}

// ----------------------------------------------------
// SCENE 8 (Scene 4.2): Gerhana Megamendung (61k -> 75k)
// ----------------------------------------------------
...
// ----------------------------------------------------
// SCENE 9 (Scene 4.3): Cuaca Sejuk - UPDATE JADI 7 DETIK (75k -> 82k)
// ----------------------------------------------------
...
// ----------------------------------------------------
// SCENE 10 - 14 (Transisi Gambar Parang)
// Waktu HitungAlphaFade SUDAH DIUPDATE agar tidak transparan!
// ----------------------------------------------------
void drawScene5(int waktu) { // 82k -> 87k
  background(#F8F4E3); imageMode(CENTER);
...
void drawScene51(int waktu) { // 87k -> 90k
  background(#B0E0E6); imageMode(CENTER);
...
void drawScene52(int waktu) { // 90k -> 92k
  background(0); imageMode(CENTER);
...
void drawScene53(int waktu) { // 92k -> 95.5k
  background(#B0E0E6); imageMode(CENTER);
...
void drawScene54(int waktu) { // 95.5k -> 99k
  background(0); imageMode(CENTER);
...
// ----------------------------------------------------
// SCENE 15 (Penutup Rekap 4 Gambar: 99k - 115k)
// ----------------------------------------------------
void drawSceneEnding(int waktu) {
  ...
  float xKiriAtas = width * 0.25;     float yKiriAtas = height * 0.25;
  float xTengahBawah = width * 0.50;  float yTengahBawah = height * 0.75;
  float xTengahAtas = width * 0.50;   float yTengahAtas = height * 0.25;
  float xKiriBawah = width * 0.25;    float yKiriBawah = height * 0.75;
...
// ----------------------------------------------------
// FUNGSI HELPER: Alpha Fade In & Out (Sangat Praktis!)
// ----------------------------------------------------
...
// ----------------------------------------------------
// SCENE 16 (Kredit: 115k ke atas)
// ----------------------------------------------------
void drawCreditScene(int waktu) {
  background(255);
  if (alphaCredit < 255) alphaCredit += 3;

  textAlign(CENTER, CENTER);
  textSize(38); fill(30, 30, 30, alphaCredit);
  text("Visualisasi Filosofi Batik", width/2, height * 0.20);
  
  textSize(72); fill(0, 0, 0, alphaCredit);
  text("DAUN", width/2, height * 0.38);
  
  stroke(0, alphaCredit); strokeWeight(1.5);
  line(width * 0.30, height * 0.52, width * 0.70, height * 0.52);
  noStroke();

  textSize(22); fill(60, 60, 60, alphaCredit);
...
void mousePressed() {
  if (mouseButton == LEFT) {
    ...
    alpha1 = 0; alpha2 = 0; alpha3 = 0; alpha32 = 0; alpha33 = 0; alphaMega = 0;
    alpha41 = 0; alpha42 = 0; alpha5 = 0; alpha52 = 0; alpha53 = 0; alpha54 = 0;
    alphaEnding1 = 0; alphaEnding2 = 0; alphaEnding3 = 0; alphaEnding4 = 0;
    alphaCredit = 0; zoom1 = 1.1;
    
    int targetWaktu = -1;
    for (int wp : waypoints) {
      if (w < wp) { targetWaktu = wp; break; }
    }
...
class KunangKunang {
  ...
  KunangKunang() { 
    startX = random(width); startY = random(height); ukuranInti = random(5, 12);
    ukuranGlow = ukuranInti * 4; kecepatanKepak = random(0.02, 0.08); offset = random(TWO_PI);
  }
  KunangKunang(float x, float y, float u) { 
    startX = x; startY = y; ukuranInti = u;
    ukuranGlow = u * 4; kecepatanKepak = 0.05; offset = 0; diamDiTengah = true;
  }
  void update() { 
    yPos = startY + sin(frameCount * kecepatanKepak + offset) * 20;
    if (!diamDiTengah) startX += random(-0.5, 0.5); xPos = startX; 
  }
  void display(float gA, float iA) { 
    noStroke();
    fill(255, 255, 50, gA); circleBentuk(xPos, yPos, ukuranGlow/2); 
    fill(255, 255, 150, iA); circleBentuk(xPos, yPos, ukuranInti/2);
  }
  void circleBentuk(float x, float y, float r) { 
    beginShape(); 
    for(float s=0; s<TWO_PI; s+=TWO_PI/16) vertex(x+cos(s)*r, y+sin(s)*r); 
    endShape(CLOSE);
  }
}
...
class AwanMegamendung {
  ...
  AwanMegamendung(int index) {
    ...
    w = imgMegamendung.width*s; h = imgMegamendung.height*s;
    int k = index % 5; int b = index / 5;
    float jX = width / 5; float jY = height / 3;
    x = (k * jX) + (jX / 2) + random(-40, 40);
    y = (b * jY) + (jY / 2) + random(-30, 30);
  }
  void display() { image(imgMegamendung, x, y, w, h); }
}